...
UZL_Task_SpawnPredictedProjectile* UZL_Task_SpawnPredictedProjectile::SpawnPredictedProjectile(
    UGameplayAbility* OwningAbility, TSubclassOf<AProjectile> ProjectileClass, TSubclassOf<AProjectile> Ser_Projectile,
    FVector SpawnLocation, FRotator SpawnRotation)
{
    ...
    Task->ServerProjectileClass = Ser_Projectile;
       ...
       if (AZero_BasePlayerController* ZeroPc = Ability->GetCurrentActorInfo()->PlayerController.IsValid() ? Cast<AZero_BasePlayerController>(Ability->GetCurrentActorInfo()->PlayerController.Get()) : nullptr)
       {
          ...
          if (!bIsNetAuthority)
          {
             ...
             AProjectile* NewProjectile = GetWorld()->SpawnActorDeferred<AProjectile>(
                 ...
                 Cast<APawn>(Ability->GetCurrentActorInfo()->AvatarActor.Get()),
                 ESpawnActorCollisionHandlingMethod::AlwaysSpawn
             );

             if (NewProjectile)
             {
                // Explicitly initialize state BEFORE FinishSpawning (which invokes BeginPlay)
                NewProjectile->InitProjectileId(FakeProjectileId);
                NewProjectile->InitFakeProjectile(ZeroPc, FakeProjectileId);
                NewProjectile->bIsFakeProjectile = true;
...
void UZL_Task_SpawnPredictedProjectile::SpawnDelayedFakeProjectile()
{
    ...
    {
       AZero_BasePlayerController* ZeroPc = DelayedProjectileInfo.ZeroPC.Get();
       FTransform SpawnTransform(DelayedProjectileInfo.SpawnRotation, DelayedProjectileInfo.SpawnLocation);

       AProjectile* NewProjectile = GetWorld()->SpawnActorDeferred<AProjectile>(
           DelayedProjectileInfo.ProjectileClass,
           SpawnTransform,
           AbilitySystemComponent->GetOwnerActor(),
           Cast<APawn>(Ability->GetCurrentActorInfo()->AvatarActor.Get()),
           ESpawnActorCollisionHandlingMethod::AlwaysSpawn
       ...
       if (NewProjectile)
       {
          // Initialize state BEFORE BeginPlay runs
          NewProjectile->InitProjectileId(DelayedProjectileInfo.ProjectileId);
          NewProjectile->InitFakeProjectile(ZeroPc, DelayedProjectileInfo.ProjectileId);
          NewProjectile->bIsFakeProjectile = true;
    ...
    FGameplayTag Activation)
{
    ...
    if (TargetData)
    {
       if (const FGameplayAbilityTargetData_ProjectileSpawnInfo* SpawnInfo = static_cast<const FGameplayAbilityTargetData_ProjectileSpawnInfo*>(TargetData))
       {
          ...
          const float ForwardPredictionTime = ZeroPc ? ZeroPc->GetForwardPredictionTime() : 0.0f;
          FTransform SpawnTransform(SpawnInfo->SpawnRotation, SpawnInfo->SpawnLocation);
          ...
          if (NewProjectile)
          {
             NewProjectile->InitProjectileId(SpawnInfo->ProjectileId);
             
             // Complete actor construction before fast-forwarding ticks
             NewProjectile->FinishSpawning(SpawnTransform);
